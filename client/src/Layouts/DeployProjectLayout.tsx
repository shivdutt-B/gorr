...
const DeployProjectLayout: React.FC<DeployProjectLayoutProps> = ({
  ...
  const repoSlugFromUrl = searchParams.get("slug");
  ...
  useEffect(() => {
    if (redeployData?.data?.project?.slug) {
      setCurrentProjectSlug(redeployData.data.project.slug);
    }
  }, [redeployData]);
  ...
  const handleDeploy = async () => {
    try {
      ...
      console.log("SLUG reDEPLOY: ", slug);
